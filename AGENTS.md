...
- **Row actions** (`f w p l c C m N n`) live on the app, and — except `n` and `w` (which spawn a workspace, *not* a cache cell) — never touch cmux/the cache. The **footer help text** for the backend-divergent keys is gated on the resolved backend (`FooterBar.BACKEND_ACTIONS`, fed `resolve_tool()` at compose): `f`/Focus + `N`/Nudge are cmux-only verbs so their hints hide on limux/none; `w`/Open is limux's only reach-a-workspace path so its hint hides on cmux (redundant with `f`). The actions stay bound and self-guard (pressing a hidden key still warns) — only the advertised hint follows the backend:
