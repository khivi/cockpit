...
- **Claude footprint** — idempotent writes by `cockpit setup`: the statusLine command + idle-pill hooks, the command templates under `~/.claude/commands/`, and the agent skills under `~/.claude/skills/<name>/SKILL.md`. Every one has an inverse in `teardown_claude_integration`; **a new install target must be added to both**, or `cockpit teardown` leaves it dangling after `brew uninstall`. **`_COCKPIT_HOOKS` is exactly two hooks** (`Stop` → `idle-pill stop`, `UserPromptSubmit` → `idle-pill prompt`), the only two with a reader. A `Stop` → `statusline` hook and three `loop=` hooks were removed and **must not return without a reader**. `install_claude_hooks`' drop pass sweeps **every event in the file**, since a retired hook lives under an event the template no longer names; an event left with no groups is deleted, not emptied. `_COCKPIT_HOOK_CMD_RE` keeps matching `statusline` to clean out older installs.
...
**Slash commands are user commands, not a plugin.** `cockpit/claude_commands/*.md` wrap `cockpit new/close/broadcast/nudge $ARGUMENTS`. **A command template documents the CLI, never reimplements it** — and so does a skill.

**Commands and skills differ only in how they are *reached*, and that is the whole rule for picking one.** A command is **typed**; a skill is **matched from its `description`** when the model is already doing something it covers. So a gesture the user asks for by name is a command, and one the agent should reach for unprompted is a skill — which is why `cockpit-diff` is the skill: a session should check for review notes left on its own work without being told to. `cockpit/claude_skills/<name>/SKILL.md` is a **directory per skill**, since Claude Code resolves `<skills-dir>/<name>/SKILL.md` — **do not** flatten it the way the command templates are flattened. A skill needs **YAML frontmatter** (`name`, `description`) or it is never loaded, and `uninstall_claude_skills` removes only our `SKILL.md`, dropping the directory **only when that leaves it empty**. `parse_args` **errors** on a bare `--context` rather than defaulting to none — an unexpanded flag means the substitution didn't happen. **Do not** teach the CLI to synthesize its own context. **They install as flat, hyphenated files** (`cockpit-new.md` → `/cockpit-new`); colon-namespacing is plugin-only. hatchling ships only **VCS-tracked** files, so a new template must be `git add`ed.
...
- `a` **ask** → **the one manual send**: a one-line modal (`AskScreen`) routed through `cmux.nudge_if_idle` with no `pref_key`, so it overrides mute/snooze while honouring every idle guard and a mid-turn session refuses it instead of having the text typed into a y/n prompt. Reaches an *existing* workspace only. Writes no cell or pill.
  ...
  **`a` is also what delivers the diff-viewer comments `cockpit diff` collects** (`lib/diff_comments.py`), because cmux folds them into the next message *its own composer* submits and a cockpit workspace is a terminal running Claude's TUI, which has no composer (measured). It is **one of two readers on one ledger** — `cockpit diff --comments` is the other — so a note goes out once whichever way it was collected. Seven rules: it **rides a message, never sends alone**; the **typed line leads, anchors follow**; it is built from the store's **structured fields, never `submissionText`**, whose fences `cmux send` would turn into Enter presses; it is **read-only on cmux's store**, recording delivery in `$COCKPIT_RUNTIME_DIR` — **do not** move that ledger into `$COCKPIT_HOME/state`; it marks delivered **only on a send the gate accepted**; the **repo-header fan-out carries none**; and lookup matches each store file's own `repoRoot`, offering **both** the worktree and its main checkout. Everything fails **open**.
...
- **`--comments` is the other half of `a`'s loop, not a second ledger.** It prints `lib/diff_comments.py`'s pending notes and calls the same `mark_delivered`, so a note is delivered once however it was collected. It offers **both** candidate roots (the worktree and `main_worktree_path`), exactly as `_send_ask` does. It opens no diff.
