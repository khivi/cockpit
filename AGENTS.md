...
`cockpit.py::_nudge_diff_comments`, on the fast tick, sends `DIFF_COMMENTS_NUDGE` (`/cockpit-diff apply`, the bundled skill) to the session sitting in a worktree that has pending notes. It rides `nudge_if_idle` like every other send — **do not** give it a second send path. Six rules:
