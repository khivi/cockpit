...
## Invariant ids — registered in `tests/invariant_ids.py`, never in this file

`tests/test_invariant_coverage.py` is the third repo-wide invariant test and the AGENTS.md → *tests* direction, where `test_comment_references.py` is the AGENTS.md → *code* one. `INVARIANTS` registers an id against a verbatim phrase from the rule; a test claims it with `@pytest.mark.covers("some.rule.id")`. **The link is the id, never a path or a symbol**, so a rename moves neither end — which is the whole reason to prefer it over naming the test in prose, as several rules above still do. Seven rules:

- **This file carries NO id markup.** An earlier cut stamped a trailing `[some.rule.id]` on each rule and parsed it back out; the registry replaced it. A rule's prose stays prose.
- **The registry entry is a verbatim substring of the rule, and a test pins that** (`test_every_registered_id_names_a_findable_rule`). It is the only route from an id back to its rule, so a paraphrase makes the rule unfindable and the id meaningless. **Do not** reword an entry into a summary.
- **That check is also the only thing that notices a DELETED rule.** The registry outlives the prose by construction, so without it a rule could be removed while its id stayed registered and its tests stayed green — the gate would guard something that no longer exists. **Do not** drop it as redundant.
...
- **The uncovered list is a WARNING, not a failure** — a rule with no test yet must not break a run that has nothing to do with it. Flipping it to an assertion is a decision for when the list is empty. A `covers()` naming an *unregistered* id is the half that fails hard.
- **`JUDGMENT_ONLY` is for a rule no test COULD hold** — one about the shape of code a future change would add. Each entry is a claim, not a backlog item, and the gate refuses an entry that turns out to have a test. **Do not** park a merely-untested rule there; that is what the warning is for.
