...
**`sidebar_tag` is opt-in per repo and applied by ONE function, `git.py::tag_workspace_name`** — tint alone stops scaling past `CMUX_COLOR_ANSI`'s sixteen names, several of which don't read apart, and a repo with no `sidebar_color` carries no repo signal at all. Five rules:
