...
**Workspace names track the worktree dir (`wt.short`), re-asserted on both ticks.** A workspace is named `wt.short` at spawn, but the name drifts — the user renames it, a closed-then-reopened PR reuses the branch, or a limux spawn lands a uuid. cockpit resolves workspaces by cwd→path (never by name), so drift was silently tolerated until the user had to run `cmux rename-workspace` by hand. `cmux.rename_workspace_if_needed` re-asserts the convention (idempotent: a no-op when the name already matches), called per-tracked/orphan workspace in the slow tick (`cycle.py::_refresh_tracked_pills`, `_refresh_orphan`) and in bulk in the fast tick (`cmux.reconcile_workspace_names`, cwd→path matched) so a drift recovers within ~30s. Like `sidebar_color` it is a cosmetic cmux-only output, not a render cell, and never a `send`. **Consequence**: a manual rename that doesn't match the worktree dir is reverted to `wt.short` on the next tick — to get a custom sidebar label, rename the worktree directory, not the workspace. **Exception — the primary checkout is exempt.** A workspace whose cwd is the repo's canonical clone (`wt.is_primary`, on a main branch) keeps its custom name: the slow-tick paths already skip it (no PR → never `tracked`; `master` ∈ `MAIN_BRANCHES`), and `reconcile_workspace_names` guards on `wt.is_primary`. Without this, a workspace parked on `master` to run skills (e.g. one the user named `morning` for morning-align) was force-renamed to the repo dir name every fast tick, and the "rename the worktree dir" escape hatch couldn't apply — the primary dir *is* the repo's canonical name. `rename-workspace` is not a pill verb, so it works on cmux and limux alike. The log line fires only when a rename actually happens (divergence), never on the steady-state no-op.
