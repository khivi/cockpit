...
**v1 owns no read/edit surface of its own.** Reviewing a diff, browsing code, and making a manual fix all happen in the user's own browser (the GitHub PR page) or editor (`code <worktree>`), opened *by hand* — cockpit ships zero launchers, zero viewers. Live on that. Let real friction, not speculation, decide which surfaces below earn a place in v2 — add or amend from use, don't pre-build.
...
- **In-app review → agent — cockpit's job, the differentiated one.** A multi-source **read-only** diff viewer — uncommitted (`git diff`), branch (`git diff <base>...HEAD`), PR (`gh pr diff <n>`), or arbitrary (`git diff A B`): one `react-diff-view` component with a source picker in front — plus read-only **full-file expansion** to reach lines the diff didn't touch. Both are **click-to-comment**; notes **batch into a single prompt** written to that worktree's agent pty ("Review → agent"), then vanish. **Ephemeral: no comment store, no threads / resolve / suggestions** — that's rebuilding GitHub *and* breaking the everything-ephemeral rule. This is KEYMAP applied to review (review intent → prompt bytes → pty): the one review surface GitHub can't offer (steer *your own* agent) and VS Code can't (it doesn't know your agent exists). GitHub's own review UI demotes to a launcher for the human-collaborator / durable-thread case only. **Hard line — NO file/dir tree, no tabs, no edit-in-place.** The tree is the first organ of an IDE and buys nothing your editor doesn't do better: to reach a reviewed file you have the diff's changed-files list; to browse unrelated code you have open-in-editor. Wanting a tree is the signal to open the editor, not grow one. cockpit renders what you *review*, never a place you *navigate*. Keep/cut test for any pixel: *routes feedback to the agent* → keep; *exists to browse/read/edit* → editor's job.
- **Escape hatches (open-in-editor / shell tile) — thin launchers, not surfaces.** `code <worktree>` for read / navigate / manual fix; a **shell** pty tile (reuses the pty machinery, spawns `$SHELL` at the worktree cwd instead of `claude`) for git surgery / conflict resolution / unblocking a stuck agent. One line each. Deferred to v2 on purpose — v1 opens these by hand so use reveals which are worth a button. The shell glance shows only the *local* tree; anything that's a PR reviews on GitHub / via the diff viewer, not the shell. **Guardrail — the shell tile is anchored to a worktree row (`{agent tile, optional shell tile}` at the same cwd), never a free-floating terminal tab.** Every tile must trace back to a row, or the 1:1:1 instrument-panel model breaks; a generic tabbed terminal is rebuilding cmux/iTerm and diluting "every pane is a tracked unit of work." Want a scratch shell unattached to any worktree? That's the OS terminal's job, not cockpit's.
