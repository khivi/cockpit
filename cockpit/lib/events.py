...
def events_supported() -> bool:
    """True when the backend is cmux and it advertises `events.v1`."""
    # ponytail: local `cmux capabilities` read; swap for the shared capability
    # helper when khivi/cmux-capabilities lands.
    if not is_cmux() or not shutil.which("cmux"):
        return False
    try:
        out = subprocess.run(
            ["cmux", "capabilities"],
        ...
        return "events.v1" in json.loads(out).get("capabilities", [])
    except Exception:
        return False
