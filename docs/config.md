...
| `dev_done` | all | per provider | The tracker value that lights the `devdone=` pill — whatever your provider calls that thing: a Linear workflow **state**, a GitHub issue **label**, a Jira **status**, a Trello **list**. Matched by name, case-insensitively. Defaults `Dev Done` (linear, jira) and `ready for review` (github); **no default** on trello, since boards name their lists arbitrarily and an unset value leaves the pill off rather than guessing. Also read by the ticket inbox (`i`), which hides a ticket sitting in this state — it is work already done, not work to start. |
| `merge_done` | linear, jira, trello | per provider | Where a delivered ticket moves when its PR merges, if `close_on_merge`. Same per-provider noun as `dev_done`. Defaults `Done` (linear, jira); **no default** on trello. GitHub has no equivalent — it closes the issue rather than moving it. Read by the ticket inbox alongside `dev_done`, for the same reason. |
| `inbox_states` | linear, jira, trello | unset | The state/list names the ticket inbox (`i`) shows — a name, or a list of them. Unset keeps the built-in filter: Linear's unstarted/started state *types* (which excludes Backlog columns), Jira's `statusCategory != Done`, every Trello list, each minus `dev_done`/`merge_done`. Set, it **is** the whole filter: only tickets in a listed state appear, and the `dev_done`/`merge_done` drop is skipped — a listed state is wanted even when it matches one of those. Declare it on an `orgs` block to cover a team's repos; a repo's own value overrides. Matched case-insensitively for Jira statuses and Trello lists, but **exactly as spelled** for Linear — the filter rides the GraphQL query, so `"backlog"` will not match a state named `Backlog`. Ignored under `tickets: github` (issues are only open/closed); preflight warns. |
...
| `board` | trello | unset | Trello board(s) this repo's cards live on — a name, or a list of them when one repo's work spans several. Routes a `cockpit new https://trello.com/c/<id>` spawn: a card short link carries no board, so this is the *whole* route (not a tiebreaker): unset on every repo → zero fetches and no routing; declared on several → one fetch resolves the card's board. It is also the ticket inbox's scope, and **there it is required** — a Trello account spans every board you were ever added to, so with none declared the inbox asks for nothing rather than listing someone else's planning board. (Linear and Jira scope by `keys`, GitHub by the repo list itself.) Matched by name, case-insensitively; cards on an **archived** board are dropped whatever you scope to. |
| `label` | trello | unset | Trello label(s) marking a card as **this** repo's work — a name, or a list of them. Routes *below* `board`, for the several-repos-one-board shape a board alone cannot express: two repos sharing one board tie on the board stage, and a label breaks the tie. A label is orthogonal to the card's *list*, so unlike routing on the list name it never competes with `inbox_states` / `dev_done` / `merge_done` for the field that says how far along a card is — the card keeps its stage flow and carries its owner throughout. A repo declaring **no** label is the board's default and takes every card no label claims, which is what keeps the common case automatic (one repo marks its work, its sibling declares nothing). Costs one fetch, and only when the board stage left more than one candidate with some repo claiming a label — so a config with no labels pays nothing. Matched by name, case-insensitively. Routing only: nothing else reads it. |
