...
@pytest.mark.covers("**Do not** re-enable any without turning the `pr` pill off")
...
@pytest.mark.covers("**Every message is collapsed to one line")
...
@pytest.mark.covers("**Giving up presses no Enter.**")
@pytest.mark.covers("**Do not** answer a slow spawn by raising the attempt count.")
...
@pytest.mark.covers("**Never** simplify the gate to trust `Needs input`.")
...
@pytest.mark.covers("**The anchor must own a live shell")
@pytest.mark.covers(
    '**Do not** "simplify" this back to using the anchor `create` returns.'
)
...
@pytest.mark.covers("**Do not** re-derive the verdict at a call site")
...
@pytest.mark.covers(
    "**Do not** widen it to a second at-rest authority for those states."
)
...
@pytest.mark.covers(
    "the funnel every close path goes through; **do not** re-implement it per call site"
)
@pytest.mark.covers('**Never call `cmux("close-workspace", …)` raw.**')
